...
export class UsersService {
  ...
  public async getUsers(query: GetUsersQueryDto){
    ...
    return hasPagination
      ? this.getUsersPaginated(query)
      : this.getAllUsers()
  }

  private async getUsersPaginated(query: GetUsersQueryDto): Promise<PaginatedResponseDto<UserResponseDto>> {
  ...
  public async updateUser(id: string, dto: UpdateUserDto): Promise<UserResponseDto> {
    ...
    try{
      const data: Prisma.UserUncheckedUpdateInput = UsersMapper.toUpdateInput(dto);
  ...
  public async updateUserRole(id: string, role: Role): Promise<UserResponseDto> {
    try{
      const user = await this.dbService.user.update({
        where: { id },
        data: { role },
